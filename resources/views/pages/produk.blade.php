<x-layouts.app title="Katalog Produk — Aksara Karya" meta-description="Lihat semua produk fiberglass Aksara Karya: tangki air, septic tank biotech, sistem IPAL, grease trap, dan banyak lagi.">
    ...
    <section class="bg-slate-900 py-16 px-4">
        <div class="max-w-7xl mx-auto text-center">
            <div class="inline-block bg-cyan-500/20 text-cyan-300 text-xs font-semibold px-3 py-1 rounded-full uppercase tracking-wide mb-4">Katalog Produk</div>
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="flex items-center gap-2 py-3 overflow-x-auto">
                @foreach(['Semua', 'Tangki', 'Sanitasi', 'IPAL'] as $i => $cat)
                    <button class="filter-btn shrink-0 px-4 py-2 rounded-lg text-sm font-medium transition-all duration-200 {{ $i === 0 ? 'bg-cyan-500 text-white' : 'bg-slate-100 text-slate-600 hover:bg-slate-200' }}"
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6" id="products-grid">
                @foreach($products as $product)
                    <a href="{{ route('produk.detail', $product['slug']) }}"
                       class="product-item group bg-white rounded-2xl border border-slate-200 overflow-hidden hover:border-cyan-300 hover:shadow-xl transition-all duration-300 flex flex-col"
                       data-category="{{ strtolower($product['category']) }}">
                        ...
                        <div class="p-6 flex flex-col flex-1">
                            <h2 class="text-lg font-bold text-slate-900 mb-2 group-hover:text-cyan-600 transition-colors duration-200">{{ $product['name'] }}</h2>
                            <p class="text-slate-500 text-sm leading-relaxed flex-1">{{ $product['short_desc'] }}</p>

                            <div class="mt-4 pt-4 border-t border-slate-100 flex items-center justify-between">
                                <div class="flex items-center gap-1.5 text-cyan-600 text-sm font-semibold group-hover:gap-3 transition-all duration-200">
                                    Detail Produk
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 8l4 4m0 0l-4 4m4-4H3"/>
                                    </svg>
                                </div>
                                <svg class="w-5 h-5 text-slate-300 group-hover:text-cyan-300 transition-colors duration-200" fill="none" stroke="currentColor" viewBox="0 0 24 24">
