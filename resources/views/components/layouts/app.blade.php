...
<body class="bg-white font-sans antialiased">
    ...
    <nav id="navbar" class="fixed top-0 left-0 right-0 z-50 transition-all duration-300 bg-white/95 backdrop-blur-sm shadow-sm border-b border-slate-100">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="flex items-center justify-between h-16">
                ...
                <div class="hidden lg:flex items-center gap-1">
                    <a href="{{ route('home') }}" class="nav-link px-4 py-2 text-sm font-medium text-slate-700 hover:text-cyan-600 hover:bg-cyan-50 rounded-lg transition-all duration-200 {{ request()->routeIs('home') ? 'text-cyan-600 bg-cyan-50' : '' }}">
                        Home
                    </a>

                    {{-- Produk Dropdown --}}
                    <div class="relative group">
                        <button class="flex items-center gap-1 px-4 py-2 text-sm font-medium text-slate-700 hover:text-cyan-600 hover:bg-cyan-50 rounded-lg transition-all duration-200 {{ request()->routeIs('produk*') ? 'text-cyan-600 bg-cyan-50' : '' }}">
                            Produk
                        ...
                        <div class="absolute top-full left-0 mt-1 w-56 bg-white rounded-xl shadow-lg border border-slate-100 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200 z-50">
                            <div class="p-2">
                                ...
                                <a href="{{ route('produk') }}" class="block px-3 py-2 text-sm font-medium text-cyan-600 hover:bg-cyan-50 rounded-lg transition-colors duration-150">
                                    Lihat Semua Produk →
                                </a>
                            </div>
                        </div>
                    ...
                    {{-- Layanan Dropdown --}}
                    <div class="relative group">
                        <button class="flex items-center gap-1 px-4 py-2 text-sm font-medium text-slate-700 hover:text-cyan-600 hover:bg-cyan-50 rounded-lg transition-all duration-200 {{ request()->routeIs('layanan') ? 'text-cyan-600 bg-cyan-50' : '' }}">
                            Layanan
                            <svg class="w-4 h-4 transition-transform duration-200 group-hover:rotate-180" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"/>
                            </svg>
                    ...
                    <a href="{{ route('tentang') }}" class="px-4 py-2 text-sm font-medium text-slate-700 hover:text-cyan-600 hover:bg-cyan-50 rounded-lg transition-all duration-200 {{ request()->routeIs('tentang') ? 'text-cyan-600 bg-cyan-50' : '' }}">
                        Tentang
                    </a>
                    <a href="{{ route('kontak') }}" class="px-4 py-2 text-sm font-medium text-slate-700 hover:text-cyan-600 hover:bg-cyan-50 rounded-lg transition-all duration-200 {{ request()->routeIs('kontak') ? 'text-cyan-600 bg-cyan-50' : '' }}">
                        Kontak
                    </a>
                </div>
        ...
        <div id="mobile-menu" class="lg:hidden hidden border-t border-slate-100 bg-white">
            <div class="px-4 py-3 space-y-1">
                <a href="{{ route('home') }}" class="block px-3 py-2.5 text-sm font-medium text-slate-700 hover:text-cyan-600 hover:bg-cyan-50 rounded-lg transition-colors duration-150">Home</a>
                <a href="{{ route('produk') }}" class="block px-3 py-2.5 text-sm font-medium text-slate-700 hover:text-cyan-600 hover:bg-cyan-50 rounded-lg transition-colors duration-150">Produk</a>
                <a href="{{ route('layanan') }}" class="block px-3 py-2.5 text-sm font-medium text-slate-700 hover:text-cyan-600 hover:bg-cyan-50 rounded-lg transition-colors duration-150">Layanan</a>
    ...
    <footer class="bg-slate-900 text-white">
        {{-- CTA Banner --}}
        <div class="bg-gradient-to-r from-slate-900 via-cyan-900 to-cyan-600 py-16 px-4">
                ...
                <div class="lg:col-span-1">
                    ...
                    <div class="flex gap-3">
                        @foreach(['M21 12a9 9 0 01-9 9m9-9a9 9 0 00-9-9m9 9H3m9 9a9 9 0 01-9-9m9 9c1.657 0 3-4.03 3-9s-1.343-9-3-9m0 18c-1.657 0-3-4.03-3-9s1.343-9 3-9m-9 9a9 9 0 019-9', 'M21 12a9 9 0 01-9 9m9-9a9 9 0 00-9-9m9 9H3m9 9a9 9 0 01-9-9m9 9c1.657 0 3-4.03 3-9s-1.343-9-3-9m0 18c-1.657 0-3-4.03-3-9s1.343-9 3-9m-9 9a9 9 0 019-9', 'M21 12a9 9 0 01-9 9m9-9a9 9 0 00-9-9m9 9H3m9 9a9 9 0 01-9-9m9 9c1.657 0 3-4.03 3-9s-1.343-9-3-9m0 18c-1.657 0-3-4.03-3-9s1.343-9 3-9m-9 9a9 9 0 019-9'] as $icon)
                            <a href="#" class="w-9 h-9 bg-slate-800 hover:bg-cyan-600 rounded-full flex items-center justify-center transition-colors duration-200">
                ...
                <div>
                    ...
                    <ul class="space-y-2.5">
                        @foreach([['Home', 'home'], ['Produk', 'produk'], ['Layanan', 'layanan'], ['Tentang', 'tentang'], ['Kontak', 'kontak']] as [$label, $route])
                            <li>
                                <a href="{{ route($route) }}" class="text-slate-400 hover:text-cyan-400 text-sm transition-colors duration-150">
                                    {{ $label }}
                                </a>
                            </li>
                        @endforeach
